

import React from 'react'
import {  ArrowRight,Users,CircleAlert,GraduationCap,ShieldCheck,LogOut} from 'lucide-react'
import { NavLink , useOutletContext} from 'react-router-dom'


const Security = () => {
    const {setshowLogoutModal}=useOutletContext()

    const options=[

      {
        title:"General",
        icon:<Users/>,
        to:"general"
      },
      {
        title:"About CertifyO",
        icon:<CircleAlert/>,
        to:"about"
      },
      {
        title:"Manage courses",
        icon:<GraduationCap/>,
        to:"/courses"
      }, {
        title:"Security",
        icon:<ShieldCheck/>,
        to:"security"
      }, 
      
    ]
    return (
      <div className="md:p-4 h-9/10 ">       
        <h1 className="text-2xl font-bold mb-4">Security</h1>
        <div className="bg-white rounded-lg shadow px-6 pt-8 h-full relative flex flex-col justify-between">
          <ul className='flex flex-col gap-3 md:gap-6'>   
            {options.map((item)=>{
              return (<NavLink to={item.to} key={item.title} className='flex px-2 hover:bg-neutral-200 rounded-xl transition transition-300 justify-between relative py-3'>
                <div className='flex gap-2 '>
                {item.icon}
                  <span className='text-sm md:text-base font-bold'>{item.title}</span>
                </div>
                <div>
                <ArrowRight></ArrowRight>
                </div>
                <div className='bg-customgrey h-[3px] w-full absolute bottom-0'></div>
            </NavLink>)
            })}  
              <div onClick={()=>setshowLogoutModal(true)} className='flex px-2 hover:bg-neutral-200 rounded-xl transition transition-300 justify-between relative py-3'>
                <div className='flex gap-2 '>
                <LogOut/>
                  <span className='text-sm md:text-base font-bold'>Logout</span>
                </div>
                <div>
                <ArrowRight></ArrowRight>
                </div>
                <div className='bg-customgrey h-[3px] w-full absolute bottom-0'></div>
            </div>       
          </ul>
          <div className='text-sm md:text-base text-center w-full text-neutral-500'>© 2025 CertifyO. All rights reserved.</div>
        </div>
      </div>
    )
  }

  export default Security
